...
public class Driver {
	...
	void displayNumberOfEmployees() {
		...
		if (numEmployee >= 1)
			System.out.println("Number of employees is: " + directory.getNumberOfBookstoreEmployees()
					+ " (Look for other console messages.)" + n + CONTINUE);
	...
	void checkIfBookIsInStock(String bookId) {
		...
		System.out.println(
				"Yes, book with bookId " + bookId + " is in stock." + " (Look for console messages.)" + n + CONTINUE);

	}

	void displayNumberOfCDsInBookstore() {

		System.out.println("Number of CD's is " + directory.getNumCDsInBookstore() + " (Look for console messages.)" + n
				+ CONTINUE);

	}

	void checkWhetherFoodItemInMarket(String foodItem) {
		System.out.println("Yes the market carries " + directory.marketCarriesFoodItem(foodItem) + n + CONTINUE);
